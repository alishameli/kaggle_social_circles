...
/**
 * Class representing an optimization problem (a data setting); taken from
 * liblinear; "bias" excluded
 * 
 * @author miafranc
 * 
 */
public class Problem {
	 ...
	 * 
	 * @param filename
	 *            The filename containing the problem in LibSVM format.
	 */
	public void loadBinaryProblem(String filename) {
		...
		try {
			...
			while ((row = r.readLine()) != null) {
				...
				// Category:
				if (elems[0].charAt(0) == '+') {// to allow for example +1 as
												// class label
	...
	public void createProblem(ArrayList<FeatureNode[]> examples, int[] classes) {
		x = new FeatureNode[examples.size()][];
		y = new int[examples.size()];
		for (int i = 0; i < examples.size(); i++) {
			x[i] = examples.get(i);
			catmap.addCategory(classes[i]);
			y[i] = 2 * classes[i] - 1; // 0,1 => -1,1
		}
		l = examples.size();
	}
	 ...
	 * 
	 * @param row
	 *            The already split row on spaces.
